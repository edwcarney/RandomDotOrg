...
export  getQuota, checkQuota, randomNumbers, randomSequence, randomStrings, randomGaussian,
        randomDecimalFractions, randomBytes, randomBitmap
...
"""
    ...
    randomNumbers(n = 100::Number, min = 1, max = 20, base = 10, parse=true, check = true, col = 5) 
...
# Examples
```
julia> randomNumbers(5, max=50, base=16)
...
julia> randomNumbers(5, max=4096, base=2, min=2048)
5-element Array{SubString{String},1}:
 "0110011101101"
 "0110101001011"
 "0100101001110"
...
julia> randomNumbers(5, max=200, numeric=false)
5-element Array{SubString{String},1}:
 "121"
 "178"
 "155"
...
```
"""
function randomNumbers(n = 100::Number; min = 1, max = 20, base = 10, numeric = true, check = true, col = 5) 
...
```
"""
function randomSequence(;min = 1::Number, max = 20::Number, col = 1, check = true)
    (min < -1f+09 || max > 1f+09 || min > max) && return "Range must be between -1e9 and 1e9"
...
"""
    ...
    randomStrings(n=10::Number, len=5; digits=true, upperalpha=true, loweralpha=true, unique=true, check=true)
...
# Examples
```
julia> randomStrings(5, 8, loweralpha=false)
5-element Array{SubString{String},1}:
 "8LPPSHYL"
 "Y3I7ILXJ"
 "0O9C41EK"
...
julia> randomStrings(3, 4, digits=false)
...
"""
    ...
    randomGaussian(n=10::Number, mean=0.0, stdev=1.0; dec=10, col=1, notation="scientific", check=true)
...
```
"""
function randomGaussian(n=10::Number, mean=0.0, stdev=1.0; dec=10, col=1, notation="scientific", check=true)
    (n < 1 || n > 10000) && return "Requests must be between 1 and 10,000 numbers"
...
"""
    randomDecimalFractions(n=10::Number; dec=10, col=2, check=true)
...
# Example
```
julia> randomDecimalFractions(5, dec=3)
5-element Array{Float64,1}:
 0.788
 0.949
 0.773
...
"""
    randomBytes(n=10::Number; format="o", check=true)

Get n random bytes.
Returns strings in binary, decimal, octal, or hexadecimal.
Returns hexadecimal numbers for "file."
...
julia> randomBytes(5, format="file")
...
"""
    randomBitmap([format="png"; width=64, height=64, save="", overwrite='n', check=true])
